 ...
 * mock-diff の viewer はこの属性を見て、選ばれた状態とその中身、そこへ至る親だけを
 * 残し、他は全部隠す。
 *
 ...
 * 見出し (`cat-head` と各節の `<h2>`) には打たない。選んだ状態の外にあるものは
 * viewer が消すので、打たなければ勝手に消える。節ごと 1 状態にした節だけは `<h2>`
 * が状態の中に入るので残るが、それはその状態の一部である。
 */
